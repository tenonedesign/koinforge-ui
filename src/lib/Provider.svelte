<script lang="ts">
	...
	{#if $user.address == ""}
		<button class="btn btn-xs sm:btn-sm" on:click={connect}>Connect to Kondor</button>
	
	{:else}
		<div class="dropdown dropdown-end ">
		<button class="btn btn-ghost btn-xs sm:btn-sm">{$user?.address?.slice(0,5)+"..."+$user?.address?.slice(-5)}</button>
