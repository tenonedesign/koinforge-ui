...
import { env, ownedPools, rcLimit, toasts, user, users, approvedPools, submittedPools, poolsOwner, nodes, ownedNodes } from "$lib/stores";
import { KoinosNode, Pool, PoolParams, Toast, User } from "$lib/types";
...
export function updateOwnedNodes(nodes: KoinosNode[], user: User) {
  ownedNodes.set(nodes.filter(x => x.owner == user.address));
}
